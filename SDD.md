...
For expression-like nonterminals, synthesized attributes:

- `X.place`: location holding the computed result (identifier, literal, or temporary)
- `X.code`: TAC sequence for computing `X.place`

For statement-like nonterminals:

- `X.code`: TAC sequence generated by `X`

Helper functions:

- `newtemp()`: returns `t1`, `t2`, `t3`, ...
- `newlabel()`: returns `L1`, `L2`, `L3`, ...
